...
export default {
  ...
  methods: {
    ...
    getSelectedStations() {
      let _selectedRQISstationsId = []
      for (let i = 0; i < this.selectedRQISstations.length; i++) {
        _selectedRQISstationsId.push(this.selectedRQISstations[i])
      }
      return _selectedRQISstationsId
    },
    ...
    toggleSelectedStationsByStationNames(
      ...
      this.stationsVectorLayer.getSource().forEachFeature(function (feature) {
        ...
        const index = self.selectedRQISstations.indexOf(station)
        if (selectedStationNames.indexOf(station.toString()) !== -1) {
          ...
          if (index === -1) {
            self.selectedRQISstations.push(station)
          }
          ...
          if (index > -1) {
            self.selectedRQISstations.splice(index, 1)
          }
        }
      })
    },
